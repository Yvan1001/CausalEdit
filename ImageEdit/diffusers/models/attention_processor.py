...
def apply_rope(xq, xk, freqs_cis):
    cos, sin = freqs_cis
    seq_length = xq.shape[-2]
    cos_seq_len = cos.shape[0]
    if cos_seq_len != seq_length:
        if cos_seq_len < seq_length:
            pad_len = seq_length - cos_seq_len
    ...
    rotary_dim = xq_.shape[-2]
    cos = cos[:, :rotary_dim]
    sin = sin[:, :rotary_dim]
    cos = cos.view(1, 1, seq_length, -1).to(xq.device)
    sin = sin.view(1, 1, seq_length, -1).to(xq.device)
